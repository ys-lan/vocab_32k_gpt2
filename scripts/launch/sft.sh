#!/usr/bin/env bash
# Stage 2/4 -- supervised fine-tuning on instruction data.
#
# Reads the pretrained checkpoint declared by `train.ckpt` in the training
# config and writes to its `work_dir`.
set -euo pipefail
...
TRAIN_CONFIG="${TRAIN_CONFIG:-configs/instruct_config.yaml}"
